...
export default function Dashboard() {
  ...
  const ultimosLancamentos = [...faturamentos, ...custos, ...despesas]
    ...
    .map(item => ({
      ...item,
      tipo: 'clienteId' in item ? 'Faturamento' : ('categoria' in item && custos.some(c => c.id === item.id) ? 'Custo' : 'Despesa')
    }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <div className="space-y-2">
            {tarefas.filter(t => t.status ? t.status !== 'Finalizada' : !t.concluida).length === 0 ? (
                ...
                <tr key={i} className="border-b border-gray-100 last:border-0">
                  ...
                  <td className="py-3">
                    <span className={cn(
                      ...
                      item.tipo === 'Faturamento' ? "bg-green-100 text-green-700" :
                      item.tipo === 'Custo' ? "bg-red-100 text-red-700" : "bg-red-100 text-red-700"
                    )}>
                      {item.tipo}
                    </span>
                  </td>
                  ...
                  <td className={cn(
                    ...
                    item.tipo === 'Faturamento' ? "text-green-600" : "text-red-600"
                  )}>
