...
import { useAppStore, Despesa as DespesaType } from '../lib/store';
import { Card, Button, Modal, Input, Select, Label } from '../components/ui';
import { formatCurrency, formatDate } from '../lib/utils';
import { Plus, Trash2 } from 'lucide-react';
import { PieChart, Pie, Cell, Tooltip, Legend, ResponsiveContainer } from 'recharts';

const CATEGORIAS = ['Aluguel', 'Internet', 'Software', 'Contador', 'Outros'] as const;
const COLORS = ['#F59E0B', '#D97706', '#B45309', '#78350F', '#FBBF24'];

export default function Despesas() {
  const { despesas, addDespesa, removeDespesa } = useAppStore();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<string | null>(null);
  const [filterMonth, setFilterMonth] = useState(new Date().toISOString().slice(0, 7));
  
  const [formData, setFormData] = useState<Omit<DespesaType, 'id'>>({
  ...
  const filteredDespesas = despesas
    .filter(d => d.data.startsWith(filterMonth))
    .sort((a, b) => new Date(b.data).getTime() - new Date(a.data).getTime());

  const totalDespesas = filteredDespesas.reduce((acc, curr) => acc + curr.valor, 0);

  const chartData = CATEGORIAS.map(cat => ({
    name: cat,
    value: filteredDespesas.filter(d => d.categoria === cat).reduce((acc, curr) => acc + curr.valor, 0)
  })).filter(d => d.value > 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div>
          <p className="text-sm text-gray-500 font-medium">Total de Despesas no Período</p>
      ...
      <Modal isOpen={!!itemToDelete} onClose={() => setItemToDelete(null)} title="Confirmar Exclusão">
        <div className="space-y-4">
          <p className="text-gray-600">Tem certeza que deseja excluir esta despesa?</p>
